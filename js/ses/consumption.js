/* ======================================================
   SES — CONSUMPTION MODE (UNIFIED WEBHOOK STRUCTURE)
====================================================== */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const consumptionInput = document.getElementById("monthly_consumption");
  ...
  function calculateTargetAC(monthlyMWh) {
    const monthlyKWh = monthlyMWh * 1000;
    let ac = (monthlyKWh / CONFIG.generationPer100kW) * 100;
    ac = roundTo50(ac);
    if (ac < CONFIG.minAC) ac = CONFIG.minAC;
    return ac;
  }
  ...
  function validateForm() {
    ...
    const monthlyMWh = Number(consumptionInput.value);
    if (!monthlyMWh || monthlyMWh < CONFIG.minMonthlyMWh) {
      isValid = false;
    }
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    const monthlyMWh = Number(consumptionInput.value);
    const targetAC = calculateTargetAC(monthlyMWh);
    ...
    if (!modulePowerKW) {
      submitBtn.innerText = "Оберіть тип панелей";
      submitBtn.disabled = false;
      return;
    }
    ...
    const formData = {
      ...
      monthly_consumption_mwh: monthlyMWh,
      calculation_mode: "consumption",
